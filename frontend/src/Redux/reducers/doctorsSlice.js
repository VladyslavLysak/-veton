...
export const fetchDoctors = createAsyncThunk('doctors/fetchDoctors', async () => {
    try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/doctors`);
...
export const fetchPopularDoctors = createAsyncThunk('doctors/fetchPopularDoctors', async () => {
    try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/doctors/popular`);
        return response.data.doctors;
    } catch (error) {
        console.log(error);
    };
...
export const createDoctor = createAsyncThunk('doctors/createDoctor', async (data, thunkAPI) => {
    ...
    try {
        return await axios.post(`${process.env.REACT_APP_BASE_URL}/doctors/add`, { ...data });
    } catch (error) {
        return rejectWithValue(existError)
    }
});

export const updateDoctor = createAsyncThunk('doctors/editDoctor', async (data, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    const { values, id } = data;
    try {
        return await axios.put(`${process.env.REACT_APP_BASE_URL}/doctors/id/${id}`, { ...values });
    } catch (error) {
        return rejectWithValue(existError);
    }
});
...
export const deleteDoctor = createAsyncThunk('doctors/deleteDoctor', async (id, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    try {
        return await axios.delete(`${process.env.REACT_APP_BASE_URL}/doctors/id/${id}`);
    } catch (error) {
...
const doctorsSlice = createSlice({
    ...
    extraReducers: {
        ...
        [updateDoctor.fulfilled]: (state, action) => {
            const newDoctor = action.payload;
            const activeIndex = state.doctors.findIndex(el => el._id === newDoctor._id);
        ...
        [deleteDoctor.fulfilled]: (state, action) => {
            state.doctors = state.doctors.filter(el => el._id !== action.payload?.data?.movie._id);
            state.isLoading = false;
        },
