...
export const createCity = createAsyncThunk('cities/createCity', async (data, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    try {
        return await axios.post(`${process.env.REACT_APP_BASE_URL}/cities/add`, { ...data });
    } catch (error) {
        return rejectWithValue(existError)
    }
});

export const editCity = createAsyncThunk('cities/editCity', async (data, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    const { values, id } = data;
    try {
        return await axios.put(`${process.env.REACT_APP_BASE_URL}/cities/id/${id}`, { ...values });
    } catch (error) {
        return rejectWithValue(existError);
    }
});
...
export const deleteCity = createAsyncThunk('cities/deleteCity', async (id, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    try {
        return await axios.delete(`${process.env.REACT_APP_BASE_URL}/cities/id/${id}`);
    } catch (error) {
...
export const selectDoctors = (state) => state.city.cities;
export const isDoctorsLoading = (state) => state.city.isLoading;
export const selectDoctorError = (state) => state.city.error;
